...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_channel=1, zero_init_residual=False):
...
def resnet18(in_channel, pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(BasicBlock, [2, 2, 2, 2], in_channel, **kwargs)
    if pretrained:
...
class resnet18_features(nn.Module):
    def __init__(self, in_channel, pretrained=False):
        ...
        self.model_resnet18 = resnet18(in_channel, pretrained)
